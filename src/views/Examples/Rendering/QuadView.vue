...
const getComHuData = (
  obj: { [key: string]: any },
  cb?: (data: { realPixelVal: string; viewType: string }) => void
) => {
  ...
  // const mapperDataIC = obj.resliceMapper.getInputConnection();
  const image = obj.resliceMapper.getInputData();
  const { viewCode } = obj;
  const extent = image.getExtent();

  obj.interactor.onMouseMove((event: any) => {
    const pos = event.position;
    picker.pick([pos.x, pos.y, 0.0], event.pokedRenderer);
    const point = picker.getPickPosition();
    // point[2] = 0;
    let realPixelVal = "";
    // const index = image.worldToIndex(point);
    const index = image.worldToIndex(point);
    for (let idx = 0; idx < 3; ++idx) {
      if (index[idx] < extent[idx * 2] || index[idx] > extent[idx * 2 + 1]) {
        console.log(index[idx], extent[idx * 2], index[idx], extent[idx * 2 + 1])
        // debugger;
        // return cb({ realPixelVal, viewType: viewCode });
      }
    }
    try {
      const pixel = image.getScalarValueFromWorld(point);
      console.log(pixel, "pixel");
      
      if (!isNaN(pixel)) realPixelVal = parseInt(pixel) + "HU";
    } catch (error) {
      console.error(error);
    }
    cb?.({ realPixelVal, viewType: viewCode });
  }, 999);
};


onMounted(() => {
  const rootContainer = containerRef.value
  ...
  // ----------------------------------------------------------------------------
  const fileUrl = '/data/volume/head-binary.vti'
  // const fileUrl =
